...
use token_acl_gate_client::{accounts::ListConfig, instructions::DeleteListBuilder};
...
pub async fn run(ctx: &AppContext, args: DeleteListArgs) -> Result<()> {
    ...
    let actual_authority = decoded.authority;
    ...
    println!(
        ...
        args.list_config, decoded.seed, decoded.mode, decoded.wallets_count
    );
